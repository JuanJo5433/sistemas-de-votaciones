...
require_once '../../../config/validate_session.php'; //validacion de la sesion
require ('../../../config/control_acces.php'); //Valida el acceso
require('../../../config/validate_type_user.php'); //valida el tipo de usuario
$connect = connection(); //conexion
$id = $_GET["id_candidato_alcalde"]; //obtiene el id 

$consulta = "SELECT * FROM candidatos_alcalde WHERE id_candidato_alcalde='$id'"; //Busca el id seleccionado
$solicitud = mysqli_query($connect, $consulta); //realiza solicitud en la BD
$row = mysqli_fetch_array($solicitud); //realiza solicitud en la BD
...
<body>
    ...
    <div class="container">
        <!-- Encabezado. -->
        <h1>Editar Candidato a la alcaldia</h1>
        <form action="update.php" class="formulario" id="formulario" method="POST" enctype="multipart/form-data">
            ...
            <input type="hidden" name="id_candidato_alcalde" value="<?= $row['id_candidato_alcalde'] ?>">
            ...
            <label class="texto_label" for="genero">Seleccione su genero: </label>
