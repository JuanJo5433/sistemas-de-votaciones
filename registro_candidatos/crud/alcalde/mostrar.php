<?php
require_once('../../../get/get_candidatos_alcalde.php'); //obtiene la informacion del candidato
require_once '../../../config/validate_session.php'; //valida la sesion
...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="../../css/estilo_mostrar1.css"> <!--Archivo de tipo css-->
    <title>Mostrar candidatos</title> <!--Titulo de la pagina-->
</head>

<body>
    <div class="btn-atras"><!--Creacion de boton atras-->
        <a href="../mostrar_candidatos.php"> <!--Redirecciona al archivo asignado-->
    ...
    <div class="container">
        <?php
        //Almacena la informacion para mostrar al usuario
        if (!empty($candidatos)) {
            foreach ($candidatos as $candidato) {
                ...
                if (!empty($foto)) {
                    ...
                    echo '<img src="' . $imageSrc . '" alt="' . $nombre . '" class="imagen" class="imagen">';
                } else {
                    echo 'Imagen no disponible';
                }
                echo '</div>';
                echo '<div class="botones">';
                echo '<a href="actualizar.php?id_candidato_alcalde=' . $id . '" class="btn-gradient">Editar</a>';
                // Agrega la confirmación de eliminación con el nombre del candidato
                echo '<a href="delete.php?id_candidato_alcalde=' . $id . '" class="btn-gradient" onclick="return confirm(\'¿Estás seguro de eliminar a ' . $nombre . '?\')">Eliminar</a>';
                echo '</div>';
                echo '</div>';
            }
        } else {
