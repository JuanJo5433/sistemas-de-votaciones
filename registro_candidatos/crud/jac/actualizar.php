...
require('../../../config/validate_type_user.php'); //Valida el ipo de usuario

$connect = connection();
//Obtiene el id del candidato
$id = $_GET["id_candidato_jac"];

$consulta = "SELECT * FROM candidatos_jac WHERE id_candidato_jac='$id'"; //Realiza una consulta en la BD
$solicitud = mysqli_query($connect, $consulta); //Envia la solicitud a la BD
$row = mysqli_fetch_array($solicitud); //Envia la solicitud a la BD
$base64Image = base64_encode($row['image']); //Transforma la imagen en codigo binario
$imageSrc = 'data:image/jpeg;base64,' . $base64Image; //Transforma la imagen en codigo binario
...
<body>
    ...
    <div class="container">
        ...
        <form action="update.php" class="formulario" id="formulario" method="POST" enctype="multipart/form-data">
            ...
            <input type="hidden" name="id_candidato_jac" value="<?= $row['id_candidato_jac'] ?>">
