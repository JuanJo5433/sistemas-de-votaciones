...
$connect = connection();
//Permite obtener el id del candidato
$id = $_GET["id_candidato_representante_sena"];

$consulta = "SELECT * FROM candidatos_representante_sena WHERE id_candidato_representante_sena='$id'"; //Se realiza la consulta enla base de datos
$solicitud = mysqli_query($connect, $consulta); //Se envia la solicitud
$row = mysqli_fetch_array($solicitud); //Se envia la solicitud
$base64Image = base64_encode($row['image']); //Transforma la imagen a codigo binario
$imageSrc = 'data:image/jpeg;base64,' . $base64Image; //Transforma la imagen a codigo binario
...
<body>
    ...
    <div class="container">
        <!-- Encabezado. -->
        <h2>Editar Candidato a la alcaldia</h2>
        <form action="update.php" class="formulario" id="formulario" method="POST" enctype="multipart/form-data">
            ...
            <input type="hidden" name="id_candidato_representante_sena" value="<?= $row['id_candidato_representante_sena'] ?>">
